...
- **prompt تفعيل البصمة (مثل البنوك):** `src/components/PasskeyPrompt.tsx` يُعرض من `AuthGuard` بعد الدخول — يظهر بطاقة "فعّل الدخول بالبصمة" مرة واحدة إذا الجهاز يدعمها ولا توجد passkey بعد ولم يُرفض سابقاً (`localStorage: ams_passkey_prompt_dismissed`). زر الدخول بالبصمة بصفحة الدخول يظهر فقط بعد التسجيل على الجهاز (hint محلي).
