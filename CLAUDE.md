...
## ⭐ أسلوب العمل — "اسأل حالك" (مهم، قاعدة دائمة)

صاحب المشروع غير مبرمج ويفضّل الفهم البصري والوظيفي على قراءة الكود. قبل أي تعديل أو إجابة:

1. **اسأل حالك أسئلة ذكية بصوت عالٍ** وجاوب عليها قبل ما تبلّش: شو هدف الشاشة؟ مين بيشوفها (زائر/تاجر/أدمن)؟ كل زر شو بيعمل؟ وين الأزرار بتختفي أو بتتغيّر حسب الدور؟ شو أسهل طريقة للمستخدم؟
2. **فصّل الشاشة زر-زر** قبل ما تغيّرها — افهم السلوك الحالي كامل، مش بس السطر اللي رح تعدّله.
3. **فكّر بصرياً:** إذا في صورة/سكرين شوت متاحة استعملها؛ وإذا لزم، اطلب من المستخدم يبعت سكرين شوت بدل ما تنبش بالكود.
4. **دليل الشاشات والأزرار** محفوظ في `docs/screen-guide.md` — حدّثه عند إضافة/تعديل أي شاشة أو زر، وارجعله لتفهم الواجهة بسرعة.
...
- `store.products.barcode` — ברקוד EAN/UPC اختياري (لماسح الكاميرا؛ المطابقة تتدرّج barcode→sku→slug فالأقدم بلا باركود يبقى يعمل)
...
- `margin.ts` — ربح/هامش% + حارس "تحت التكلفة"/"هامش ضعيف" + تسعير جماعي + `recommendPrice` (توصية سعر ذكية: ثبات سعر الزبون التاريخي، لا تحت رصيف الربحية، وإلا مחيرון/كلفة+). تستخدمه: `/admin/customer-prices`، `/admin/quotes` (شريحة "💡 מומלץ")، `/admin/products`.
- `messages.ts` — قوالب واتساب (تذكير دفع، عرض، فاتورة، استرجاع زبون، أمر شراء) فوق `waLink`. تستخدمه: dealers، invoice، quotes، purchase-orders، OpportunitiesPanel، Customer 360.
- `activity.ts` — كشف الزبائن المتوقّفين والمخزون الميت (recency). تستخدمه: `OpportunitiesPanel` (OpsCenter)، `briefing.ts`.
- `cartFromOrder.ts` — `resolveReorder`: يحوّل عناصر طلب سابق لأسطر سلة. تستخدمه: `/account/order`، `/account/orders`.
- `barcode.ts` — تطبيع مطابقة الباركود/QR لمنتج (barcode→sku→slug ثم contains فريد) + `SCAN_FORMATS`. تستخدمه: `BarcodeScanner`، كتالوج الموزّع، مخزون الأدمن، محرّر المنتج.
- `customer360.ts` — `buildCustomer360`: يدمج `ar`+`pulse`+`reorder` ويشتق درجة/مستوى خطر (chوب متأخر/خمول مقابل وتيرة/تراجع إنفاق). تستخدمه: `/admin/customer`.
- `briefing.ts` — `buildBriefing`: قائمة مهام اليوم المرتّبة (طلبات/تسجيلات للموافقة، ديون متأخرة، مخزون ن'מ, عروض معلّقة/توشك تنتهي، زبائن نائمون) + `greeting`. تستخدمه: `DailyBriefing` على `/admin`.

### مزايا الذكاء التشغيلي الجديدة (واجهات)
- **ماسح باركود/QR** (`BarcodeScanner.tsx`): `BarcodeDetector` الأصلي + كاميرا، fallback إدخال يدوي إذا غير مدعوم. مدمج في: كتالوج الموزّع (مسح→يفتح المنتج أو يبحث)، مخزون الأدمن (مسح→يختار المنتج بفورم الحركة)، محرّر المنتج (مسح→يملأ `barcode`).
- **تيك لكوח 360°** (`/admin/customer?id=`): بطاقة موحّدة لكل زبون (خطر، گيول חוב، نبض إنفاق، منتجاته المتكرّرة، آخر الطلبات، أزرار واتساب حسب الحالة). مدخلها من قائمة `/admin/dealers` ("תיק לקוח"). query-param route (صديق static export، بلا generateStaticParams).
- **נבץ היום** (`DailyBriefing.tsx`): بريفينغ صباحي قابل للضغط أعلى `/admin` فوق `buildBriefing`.
